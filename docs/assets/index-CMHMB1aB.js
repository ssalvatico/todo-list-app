...
</footer>`,C={clearCompleted:`.clear-completed`,newTodoInput:`#new-todo-input`,count:`#pending-count`,todoList:`.todo-list`,filters:`.filtro`,toggleAll:`#toggle-all`};f.initStore(),(e=>{let t=()=>{g(C.count)},n=()=>{m(C.clearCompleted)},r=()=>{let e=f.getTodos(f.getCurrentFilter()).reverse();x(C.todoList,e)},i=()=>{t(),n(),r()};(()=>{let t=document.createElement(`div`);t.innerHTML=S,document.querySelector(e).append(t),r()})();let a=document.querySelector(C.clearCompleted),o=document.querySelector(C.newTodoInput),s=document.querySelectorAll(C.filters),c=document.querySelector(C.todoList),l=document.querySelector(C.toggleAll);o.addEventListener(`keyup`,e=>{e.key===`Enter`&&e.target.value.trim().length!==0&&(f.addTodo(e.target.value),e.target.value=``,i())}),c.addEventListener(`click`,e=>{let t=e.target.closest(`[data-id]`);if(!t)return;let n=t.getAttribute(`data-id`);e.target.classList.contains(`destroy`)?f.deleteTodo(n):f.toggleTodo(n),i()}),a.addEventListener(`click`,e=>{f.deleteCompleted(),i()}),s.forEach(e=>{e.addEventListener(`click`,e=>{s.forEach(e=>e.classList.remove(`selected`)),e.target.classList.add(`selected`),f.setFilter(e.target.dataset.filter),r()})}),l.addEventListener(`click`,e=>{y(),i()})})(`#app`),`serviceWorker`in navigator&&navigator.serviceWorker.register(`${import.meta.env.BASE_URL}/sw.js`).then(e=>{console.log(`SW registrado, scope:`,e.scope)}).catch(e=>{console.error(`Error registrando el SW:`,e)});